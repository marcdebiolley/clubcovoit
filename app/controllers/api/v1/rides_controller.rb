class Api::V1::RidesController < ApplicationController
  ...
  # GET /api/v1/rides
  def index
    rides = Ride.order(created_at: :desc).select(:id, :title, :date, :time, :origin, :destination, :seats_total, :seats_taken, :note, :invite_code, :created_at)
    render json: rides
  end
  ...
  def create
    ...
    if params[:password].present?
      ride.set_password(params[:password].to_s)
    end
    if ride.save
