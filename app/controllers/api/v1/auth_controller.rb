class Api::V1::AuthController < ApplicationController
  def signup
    ...
    return render json: { error: 'EMAIL_EXISTS' }, status: :conflict if User.exists?(email: email)

    user = User.new(email: email, display_name: display_name, password: password, first_name: first_name,
                    last_name: last_name)
    if user.save
      token = encode_token({ user_id: user.id, exp: 7.days.from_now.to_i })
      render json: { token: token, user: { id: user.id, email: user.email, display_name: user.display_name, first_name: user.first_name, last_name: user.last_name } },
             status: :created
    else
