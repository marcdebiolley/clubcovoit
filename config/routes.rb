Rails.application.routes.draw do
  ...
  resources :clubs, param: :share_token, only: %i[index show] do
    # Nested events (minimal for now)
    resources :events, only: [:show]

    # Drivers/Passengers endpoints (stubs that will delegate to API or ActiveRecord)
    resources :events, only: [] do
      resources :drivers, only: %i[create destroy]
      resources :passengers, only: %i[create destroy]
    end
  ...
  namespace :api do
    namespace :v1 do
      ...
      resources :rides, only: %i[index create show update destroy] do
      ...
      resources :cars, only: %i[update destroy]
      resources :participants, only: %i[destroy update]
